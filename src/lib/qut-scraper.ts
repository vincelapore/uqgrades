...
export async function fetchQUTCourseAssessment(
  ...
  console.log("[QUT Scraper] Starting fetchQUTCourseAssessment for:", unitCode);
  ...
  if (
    ...
    throw new Error(`Unit ${unitCode} not found for ${semester.semester} ${semester.year}.`);
  }
  ...
  console.log("[QUT Scraper] Found", h4Elements.length, "h4 elements with assessment-task id");
  ...
  h4Elements.each((_: number, el: Element) => {
    ...
    console.log("[QUT Scraper] Found assessment heading:", name, "| id:", elementId);
    ...
    if (headingIdx >= 0) {
      const afterHeading = parentHtml.substring(headingIdx + headingOuterHtml.length);
    ...
    const weightMatch = contentHtml.match(/<strong>Weight:<\/strong>\s*(\d+(?:\.\d+)?)/i) ||
    ...
    // Check for hurdle
    if (contentHtml.toLowerCase().includes("hurdle") || 
    ...
    if (name && (weight === "pass/fail" || (typeof weight === "number" && weight > 0))) {
      // Check for duplicates (QUT pages sometimes have the same assessment listed twice)
      const isDuplicate = items.some(
        (existing) => existing.name === name && existing.weight === weight
      );
      if (!isDuplicate) {
        console.log("[QUT Scraper] Parsed item:", { name, weight, dueDate, isHurdle });
        items.push({
          name,
          weight,
          dueDate,
  ...
  if (items.length === 0) {
    console.log("[QUT Scraper] No assessment-task h4 elements found, trying alternative parsing...");
    
    // Look for assessment section and parse Weight/Due patterns
    const bodyText = $("body").html() || "";
    
    // Pattern: "Assessment: Name" followed by "Weight: X" 
    const assessmentSections = bodyText.split(/<h4[^>]*id="assessment-task/i);
    
    assessmentSections.slice(1).forEach((section) => {
      // Extract name from heading
      const nameMatch = section.match(/>Assessment:\s*([^<]+)/i) ||
                        section.match(/>([^<]+?)<\/h4>/i);
      if (!nameMatch) return;
      ...
      // Extract weight
      const weightMatch = section.match(/<strong>Weight:<\/strong>\s*(\d+(?:\.\d+)?)/i) ||
      ...
      // Extract due date
      const dueMatch = section.match(/Due\s*\([^)]*\):\s*([^<]+)/i) ||
                       section.match(/Due:\s*([^<]+)/i);
      const dueDate = dueMatch ? dueMatch[1].trim() : null;
      
      if (name && (typeof weight === "number" && weight > 0)) {
        console.log("[QUT Scraper] Parsed item (fallback):", { name, weight, dueDate });
  ...
  if (items.length === 0) {
    ...
    console.error("[QUT Scraper] Page HTML sample:", html.substring(0, 3000));
    throw new Error(
  ...
  console.log("[QUT Scraper] Successfully parsed", items.length, "assessment items");
  return result;
}
...
export async function fetchQUTDeliveryModes(
  ...
  console.log("[QUT Scraper] Fetching delivery modes for:", unitCode, year, semester);
  ...
  try {
    ...
    const errorMessage = $(".alert-danger, .error-message, .no-results").text().trim();
    ...
    if (hasContent) {
      console.log("[QUT Scraper] Found unit, returning default Internal mode");
      return [
        {
          delivery: "Internal",
          location: "Brisbane",
